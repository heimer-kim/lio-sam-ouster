...
def check_diff():
    if imu_time is not None and lidar_time is not None:
        ...
        #diff = abs((imu_msg.header.stamp - lidar_msg.header.stamp).to_sec())
...
if __name__ == '__main__':
    rospy.init_node('timestamp_diff_checker')
    rospy.Subscriber("/imu/data_fixed", Imu, imu_callback)
    rospy.Subscriber("/points_filtered", PointCloud2, lidar_callback)
    rospy.spin()
